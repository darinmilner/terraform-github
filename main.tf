resource "github_repository" "terraform_repo" {
  # count       = var.repo-count
  for_each    = var.repo_names
  name        = "terraform-exam-review-${each.key}"
...
resource "terraform_data" "repo_clone" {
  ...
  # provisioner "local-exec" {
  #   when    = destroy
  #   command = "powershell -Command \"Remove-Item -Force '${github_repository.terraform_repo[each.key]}'\""
  # }

  # # linux/mac
  # provisioner "local-exec" {
  #   when    = destroy
  #   command = "rm -f  ${self.name}"
  # }
}
...
resource "github_repository_file" "readme" {
  ...
  branch     = "main"
  file       = "README.md"
  content = templatefile("templates/readme.tf.tpl", {
    env  = var.env,
